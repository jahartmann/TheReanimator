...
export default async function ServerDetailPage({
    ...
    return (
        <div className="space-y-6">
            ...
            <Suspense fallback={
                <Card className="border-muted/60 bg-muted/5 p-20 flex flex-col items-center justify-center space-y-4">
                    <Loader2 className="h-8 w-8 text-primary animate-spin" />
                    <p className="text-muted-foreground animate-pulse text-sm font-medium tracking-wide">Analysiere Host & lade virtuelle Maschinen...</p>
                </Card>
            }>
                <ServerTabsContent server={server} />
            </Suspense>
        </div>
    );
}

// Nested Async Component to non-block the main layout frame
async function ServerTabsContent({ server }: { server: ServerItem }) {
    const [info, vms, availableTags, scanResults, otherServers] = await Promise.all([
        getServerInfo(server),
        getVMs(server.id),
        getTags(),
        getScanResults(server.id),
        db.prepare('SELECT id, name FROM servers WHERE id != ?').all(server.id) as { id: number; name: string }[]
    ]);
    ...
    return (
        <Tabs defaultValue="overview" className="space-y-6">
            ...
            <TabsContent value="health">
                <ServerHealth initialResults={scanResults} serverId={server.id} />
            </TabsContent>
            ...
            <TabsContent value="network">
                <ServerNetwork info={info} serverId={server.id} />
            </TabsContent>
