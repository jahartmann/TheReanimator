/**
 * Login page for the React SPA.
 * Mirrors the design and logic of the original Next.js login page,
 * but calls POST /api/auth/login instead of the Server Action.
 */
...
// UI components (from existing shadcn setup)
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import {
  Loader2, Lock, User, Key, AlertCircle, Shield, Zap, Eye, EyeOff,
} from 'lucide-react';
...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (!authLoading && user && !user.force_password_change) {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      const result = await apiCall<{ success: boolean; requiresPasswordChange?: boolean; error?: string }>(
        '/api/auth/login',
        { method: 'POST', body: JSON.stringify({ username, password }) }
      );

      if (result.success) {
        await refetch(); // Re-fetch user from /api/auth/me
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="space-y-2 text-center lg:text-left">
            ...
            <p className="text-muted-foreground">
              {showPasswordChange ? t('newPassword') : t('username')}
            </p>
          </div>
